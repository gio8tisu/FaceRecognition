...
"""
Usage:
  create_all_models.py <rootName> <outPath> [--width=<wd>] [--feat=<f>] [--numCoeffs=<nc>]
...
if __name__ == '__main__':
    ...
    num_coeffs  = int (args['--numCoeffs'])        # Number of coefficients to keep for feature vectors
    ...
    for dirName, subdirList, fileList in os.walk(path_name):
        ...
        for fname in sorted(fileList):
            ...
            if not(person_name in model_dict):
                model_dict[person_name] = model.Model(id = ident, name = person_name)
                ident = ident + 1
